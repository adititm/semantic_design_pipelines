...
from semantic_design_pipelines.core.config import (
    af3_run_config,
    generation_settings,
    load_yaml,
    resolve_output_dir,
)
from semantic_design_pipelines.core.prompts import Prompt, read_prompts
...
def build_constraint_chain(data: dict[str, Any], output_dir: Path) -> Any:
    ...
    screen_config = AlphaFold3MonomerScreenConfig(
        ...
        # See semantic_design_pipelines/calibration/results/junk_vs_evo/.
    ...
    if data.get("cofold_target_fasta"):
        from semantic_design_pipelines.calibration.build_tadb_set import read_fasta
...
def run_pipeline(config_path: Path, output_root: str | None = None) -> None:
    ...
    Args:
        ...
        output_root: Overrides where the run writes; see
            :func:`semantic_design_pipelines.core.config.resolve_output_dir` for precedence.
